...
type TargetItem = {
  ...
  export default function TargetsPage() {
    ...
    const { data: subjectCatalog } = useQuery({
      ...
      queryFn: async () => (await api.get(`/subject/${draftSubjectKey}/catalog`)).data.subject as SubjectCatalogResponse['subject'],
    ...
    const subjectOptions = useMemo(() => {
      ...
      if (!query) return selectedSubjects;
      return selectedSubjects.filter((subject) => subject.toLowerCase().includes(query));
    }, [selectedSubjects, subjectSearch]);

    const topicOptions = useMemo(() => {
      const titles = new Set<string>();
      const modules = subjectCatalog?.modules ?? [];

      modules.forEach((module) => {
        titles.add(module.title);
        module.topics.forEach((topic) => {
          titles.add(topic.title);
          topic.subtopics.forEach((subtopic) => titles.add(subtopic.title));
        });
      });
    ...
    useEffect(() => {
      if (!draftTopic && topicOptions[0] && !useCustomTopic) {
    ...
    const createTarget = useMutation({
      mutationFn: async () => {
        ...
        return (await api.post('/targets', payload)).data.target as TargetItem;
      },
      onSuccess: async () => {
    ...
    const grouped = useMemo(() => ({
      Pending: (targetsData ?? []).filter((item) => item.status === 'Pending'),
      'In Progress': (targetsData ?? []).filter((item) => item.status === 'In Progress'),
      Completed: (targetsData ?? []).filter((item) => item.status === 'Completed')
    }), [targetsData]);
                  ...
                  {grouped[status].map((item) => (
                    <div key={item.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                      <div className="mb-2 flex items-start justify-between gap-2">
                        ...
                        <Badge className={item.priority === 'High' ? 'border-pink-400/40 bg-pink-500/20 text-pink-100' : item.priority === 'Medium' ? 'border-fuchsia-400/40 bg-fuchsia-500/20 text-fuchsia-100' : 'border-white/10 bg-white/5 text-white/80'}>{item.priority}</Badge>
                      </div>
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <Button size="sm" variant="secondary" onClick={() => void updateTarget.mutateAsync({ id: item.id, data: { progress: Math.min(100, item.progress + 10) } })}>+10%</Button>
                        <Button size="sm" variant="secondary" onClick={() => void updateTarget.mutateAsync({ id: item.id, data: { progress: Math.max(0, item.progress - 10) } })}>-10%</Button>
