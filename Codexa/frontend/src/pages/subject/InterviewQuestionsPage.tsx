...
export default function InterviewQuestionsPage() {
  ...
  const questions = (catalog?.rows ?? []).map((row) => ({
    id: `${row.topic}::${row.subtopic || row.topic}`,
    question: row.subtopic || row.topic,
    topic: row.topic,
    difficulty: row.difficulty,
    resource: row.interviewResource,
    hint: row.subtopic || row.module
  }));

  const isCompleted = (id: string) => normalizeProgressStatus(progressByTitle.get(makeProgressTitle('interview', id))?.status) === 'done';
  const isAttempted = (id: string) => {
    const status = normalizeProgressStatus(progressByTitle.get(makeProgressTitle('interview', id))?.status);
    return status === 'attempted' || status === 'done';
  };

  const updateProgress = async (id: string, nextStatus: 'attempted' | 'done' | 'todo') => {
    if (!subjectKey) return;
    const title = makeProgressTitle('interview', id);
  ...
  return (
    <AppShell>
      <div className="space-y-6 py-8">
        <div>
          <h1 className="text-3xl font-semibold">{catalog?.title ?? 'Subject'} — Interview Questions</h1>
          <p className="mt-2 text-sm text-white/60">Dataset-driven interview questions with saved progress.</p>
        </div>

        <Card>
          <CardHeader>
            <CardTitle>{catalogLoading ? 'Loading interview questions...' : 'Interview question set'}</CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-white/70">
            {questions.map((item) => {
              const attempted = isAttempted(item.id);
                ...
                <div key={item.id} className="rounded-2xl border border-white/10 bg-white/5 p-4">
                  <div className="flex items-start gap-4">
                    <input
                      ...
                      aria-label={`Mark ${item.question} complete`}
                    />
                    <div className="flex-1 space-y-3">
                      <div className="flex flex-wrap items-start justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs uppercase tracking-[0.24em] text-white/40">{item.hint}</p>
                        </div>
