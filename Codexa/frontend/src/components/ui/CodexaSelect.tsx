...
/**
 * Codexa themed select.
 *
 * Fix: robust value matching and safe placeholder rendering.
 * This component expects native <option value="...">...</option> children.
 */
export function CodexaSelect({
  ...
  const selected = options.find((o) => String(o.props.value ?? '') === currentValue);
  const label = selected ? normalizeLabel(selected.props.children) : (placeholder ?? '');
          ...
          <div className="max-h-60 overflow-auto p-2">
            {options.length === 0 ? (
              ...
              options.map((opt) => {
                ...
                const optLabel = normalizeLabel(opt.props.children);
                const selectedNow = optValue === currentValue;
